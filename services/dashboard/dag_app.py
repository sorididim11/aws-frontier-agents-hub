#!/usr/bin/env python3
"""
Investigation DAG — standalone Flask app for investigation flow visualization.
Separate from main dashboard to avoid rollback conflicts.

Run: python dag_app.py [--port 5002]
"""
import json
import os
import sys
...
def _boto_session():
    """Create boto3 session with the correct profile."""
    import boto3
...
@app.route("/api/render-report", methods=["POST"])
def render_report():
    """Frontend posts rendered DAG state for debugging. Printed to server console."""
    data = request.get_json(silent=True) or {}
    ...
    # Check: obs with target/resource/evidence
    empty_obs = []
    for n in nodes:
...
def api_history():
    """List investigation history from DynamoDB."""
...
@app.route("/api/agent-space-info")
def api_agent_space_info():
    """Get agent space info and validate connection."""
...
def api_journal_raw():
    """Fetch raw journal records from Agent Space."""
...
def api_journal_analyze():
    """Fetch journal records, then ask Bedrock to reclassify at signal level."""
    import re
    task_id = request.args.get("task_id")
    space_id = request.args.get("space_id", AGENT_SPACE_ID)
    if not task_id:
...
{{
  ...
  "causal_chain": [
    {{"step": 1, "service": "ServiceName", "event": "무엇이 일어났나 (Korean)"}}
  ],
  "root_cause": {{
...
- Causal chain shows the propagation path of the confirmed root cause"""
...
@app.route("/api/dag-verify")
def api_dag_verify():
    """Build expected DAG model server-side for verification.
    Returns node counts, edges, N:N connections, and diagnostics."""
    import re
    task_id = request.args.get("task_id")
    space_id = request.args.get("space_id", AGENT_SPACE_ID)
    if not task_id:
    ...
    # --- 1. Finding dedup (same id → keep highest type rank, union supporting) ---
    deduped = {}
    ...
    edge_count = (
        len(observations)  # alarm → obs
        + len(obs_finding_edges)  # obs → finding
        + len(chain_edges)  # finding → finding
        + len(orphan_obs)  # obs → terminated
    )
...
if __name__ == "__main__":
    ...
    print(f"🔍 Investigation DAG running on http://localhost:{args.port}")
